...
std::vector<std::vector<float>>  Tracker::pixel_to_camera_frame(std::vector<std::vector<int>> prediction_pixels) {
      std::vector<std::vector<float>> coordinates;
      for (auto & current_pixel : prediction_pixels){
        float offset_from_center = (current_pixel[1]-((float)_resolution[1]/2)) * _pixel_size; //Converting pixel_offset to height in focal plane
        ...
        float x_from_center = (current_pixel[0]-640) * 2.8;
...
cv::Mat Tracker::plot_coordinates(std::vector<std::vector<int>> prediction_pixels, std::vector<std::vector<float>> coordinates, cv::Mat frame){
   //Plot points on prediction
   for(int i = 0;i<prediction_pixels.size();i++){
    cv::Point pt = cv::Point(prediction_pixels[i][0], prediction_pixels[i][1]);
